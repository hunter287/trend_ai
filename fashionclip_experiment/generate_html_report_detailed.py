#!/usr/bin/env python3
"""
Генерация HTML отчёта для сравнения Ximilar vs FashionCLIP (Детализированная версия)
"""
...
# Параметры
INPUT_FILE = 'data/fashionclip_results_detailed.json'
OUTPUT_FILE = 'comparison_report_detailed.html'

def load_results():
    """Загрузить результаты"""
    if not os.path.exists(INPUT_FILE):
        print(f"❌ Файл {INPUT_FILE} не найден!")
        print(f"   Сначала запустите: python run_fashionclip_detailed.py")
        sys.exit(1)
...
<head>
    ...
    <title>FashionCLIP vs Ximilar - Детализированное Сравнение</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        ...
        <div class="subtitle">✨ Детализированное Сравнение</div>
        <div class="metadata">
            <p>Эксперимент по сравнению моделей распознавания одежды</p>
            <p>Создано: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
            <p><strong>FashionCLIP: 138 категорий, 62 цвета, 51 материал, 59 стилей</strong></p>
        </div>
        <div class="stats">
            <div class="stat-card">
...
"""
                for attr in attrs[:5]:
                    ...
                    if conf is None or (isinstance(conf, float) and (conf != conf)):  # Check for NaN
                        ...
                        conf_class = 'high' if conf > 0.7 else 'medium' if conf > 0.4 else ''
                    html += f"""
                                <div class="attribute-tag">
                                    {attr['name']}
                                    <span class="confidence {conf_class}">{confidence_pct}</span>
                                </div>
"""
                html += """
                            </div>
        ...
        html += """
                    ...
                    <div class="result-column fashionclip">
                        <h3>
                            ...
                            FashionCLIP
                            <span class="badge">Детализированная</span>
                        </h3>
"""
...
"""
    ...
    <div class="header" style="margin-top: 30px;">
        ...
        <p style="margin-top: 15px; color: #666; font-size: 0.9em;">
            Оценки будут сохранены в файл evaluations_detailed.json
        </p>
    </div>
    ...
    <script>
        function saveEvaluations() {
            ...
            localStorage.setItem('fashionclip_evaluations_detailed', JSON.stringify(evaluations));
            ...
            a.href = url;
            a.download = 'evaluations_detailed.json';
            a.click();
        ...
        window.addEventListener('load', function() {
            const saved = localStorage.getItem('fashionclip_evaluations_detailed');
            if (saved) {
...
def main():
    """Основная функция"""
    print(f"\n📊 Генерация HTML отчёта (детализированная версия)\n")
